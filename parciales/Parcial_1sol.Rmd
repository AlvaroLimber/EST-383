...
renderTable({
  eval(parse(text=paste0("zz<-","input$ssmun",input$dep)))
  mun[mun$Departamento==names(input$dep) & mun$Municipio==zz,]
})
renderPlot({
  eval(parse(text=paste0("zz<-","input$ssmun",input$dep)))
  bb<-mun[mun$Departamento==names(input$dep) & mun$Municipio==zz,]
